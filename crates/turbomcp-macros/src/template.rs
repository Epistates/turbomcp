...
pub fn generate_template_impl(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #[doc(hidden)]
        #[allow(non_snake_case)]
        fn #handler_fn_name(&self, uri: String, parameters: std::collections::HashMap<String, serde_json::Value>, context: turbomcp::RequestContext) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<String, ::turbomcp::__macro_support::turbomcp_server::McpError>> + Send + '_>> {
            Box::pin(async move {
