...
use turbomcp_dpop::{redis_storage::RedisNonceStorage, DpopError, NonceStorage};
...
async fn create_redis_storage() -> Result<RedisNonceStorage, DpopError> {
    ...
    println!(
        ...
        redis_url.replace("turbomcp_test_password", "***")
    );
...
async fn test_redis_nonce_storage_connection() {
    ...
    println!(
        "✅ Redis connection successful - found {} nonces",
        stats.total_nonces
    );
}
...
async fn test_redis_nonce_lifecycle() {
    ...
    let stored = storage
        ...
        .await
        .expect("Failed to store nonce in Redis");
    ...
    let stored_again = storage
        ...
        .await
        .expect("Failed to check nonce replay in Redis");

    assert!(
        !stored_again,
        "Duplicate nonce storage should fail (replay protection)"
    );
    ...
    assert!(
        ...
        "Stats should reflect stored nonces"
    );
    println!(
        "✅ Redis stats: {} total nonces, {} active",
        stats.total_nonces, stats.active_nonces
    );
}
...
async fn test_redis_nonce_expiration() {
    ...
    let stored = storage
        ...
        .await
        .expect("Failed to store expiring nonce");
...
async fn test_redis_storage_stats() {
    ...
    for i in 0..5 {
        ...
        storage
            ...
            .await
            .expect("Failed to store nonce for stats test");
    }
    ...
    let redis_backend_found = updated_stats
        .additional_metrics
        .iter()
...
async fn test_redis_nonce_cleanup() {
    ...
    assert_eq!(
        cleaned, 0,
        "Redis cleanup should return 0 (TTL handles expiration)"
    );
