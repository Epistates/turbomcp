...
use crate::{DpopError, NonceStorage, Result};
#[cfg(feature = "redis-storage")]
...
impl NonceStorage for RedisNonceStorage {
    async fn store_nonce(
        ...
        let serialized = serde_json::to_string(&stored_nonce)
            .map_err(|e| DpopError::StorageError {
    ...
    async fn get_usage_stats(&self) -> Result<crate::StorageStats> {
        let client = self.client.clone();
        ...
        }).await.map(|(nonce_count, jti_count)| {
            crate::StorageStats {
