...
#[cfg(feature = "dpop")]
use turbomcp_dpop::{
    DpopAlgorithm, DpopError, DpopKeyManager, DpopProof, DpopProofGenerator, ErrorSeverity,
...
async fn test_rfc_9449_compliance_proof_structure() -> Result<()> {
    ...
    assert!(
        header["jwk"].is_object(),
        "jwk must be present and be an object"
    );
    ...
    assert_eq!(
        ...
        "htu must match cleaned URI"
    );
    assert!(
        payload["iat"].is_number(),
        "iat must be present and numeric"
    );
...
async fn test_cryptographic_security_all_algorithms() -> Result<()> {
    ...
    for algorithm in algorithms {
        ...
        println!(
            "✅ Algorithm {} cryptographic security validated",
            algorithm
        );
    }
...
async fn test_replay_attack_prevention_concurrent() -> Result<()> {
    ...
    let proof_gen = Arc::new(DpopProofGenerator::with_nonce_tracker(
        key_manager,
        nonce_tracker,
    ));
...
async fn test_clock_skew_attack_resistance() -> Result<()> {
    ...
    match expired_result.unwrap_err() {
        DpopError::ClockSkewTooLarge {
            skew_seconds,
            max_skew_seconds,
        } => {
            assert!(
                skew_seconds > max_skew_seconds,
                "Clock skew should exceed maximum"
            );
            assert_eq!(
                max_skew_seconds, 300,
                "Max skew should be 300 seconds per RFC"
            );
        }
...
async fn test_nonce_exhaustion_prevention() -> Result<()> {
    ...
    for i in 0..NONCE_COUNT {
        ...
        let payload_json =
            URL_SAFE_NO_PAD
                .decode(jwt_parts[1])
                .map_err(|e| DpopError::SerializationError {
                    reason: e.to_string(),
                })?;
        let payload: serde_json::Value = serde_json::from_slice(&payload_json)?;
    ...
    println!(
        "✅ Nonce exhaustion prevention validated - {} unique nonces generated",
        NONCE_COUNT
    );
    Ok(())
}
...
async fn test_http_binding_attack_prevention() -> Result<()> {
    ...
    let wrong_host_result = proof_gen
        .validate_proof(
            &proof,
            "POST",
            "https://malicious.example.com/sensitive",
            None,
        )
        .await;
...
async fn test_concurrent_key_operations_safety() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        proof_handles.push(tokio::spawn(async move {
            pg.generate_proof(
                "GET",
                &format!("https://api.example.com/concurrent{}", i),
                None,
            )
            .await
        }));
...
async fn test_jwt_manipulation_resistance() -> Result<()> {
    ...
    assert!(
        ...
        "Algorithm downgrade attack must be prevented"
    );
...
async fn test_complete_attack_scenario_simulation() -> Result<()> {
    ...
    let token_substitution_result = proof_gen
        ...
        .await;
    assert!(
        token_substitution_result.is_err(),
        "Token substitution must be detected"
    );
    ...
    let endpoint_confusion_result = proof_gen
        .validate_proof(
            &legitimate_proof,
            "POST",
            "https://api.bank.com/account",
            Some(legitimate_token),
        )
        .await;
    assert!(
        endpoint_confusion_result.is_err(),
        "Endpoint confusion must be detected"
    );
    ...
    let method_confusion_result = proof_gen
        ...
        .await;
    assert!(
        method_confusion_result.is_err(),
        "Method confusion must be detected"
    );
    ...
    assert!(
        fresh_result.valid,
        "Fresh legitimate request must succeed after attacks"
    );

    println!(
        "✅ Complete attack scenario simulation - All attacks prevented, legitimate flow preserved"
    );
    Ok(())
}
