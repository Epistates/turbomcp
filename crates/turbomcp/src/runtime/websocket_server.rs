//! WebSocket Server Runtime - Full MCP 2025-06-18 over WebSocket
//!
//! **Status**: Production implementation following MCP 2025-06-18 spec
//!
//! This module implements the complete MCP protocol over WebSocket transport with
//! native full-duplex bidirectional communication for server→client requests
//! (sampling, elicitation, roots, ping) without the complexity of HTTP's multi-session
//! architecture.
//!
//! ## Architecture
//!
...
//! |----------------|---------------------|
//! | Multi-session (many clients) | Per-connection (1 client) |
//! | Factory pattern for sessions | Direct dispatcher per connection |
//! | SSE for server→client | Native WebSocket for both directions |
//! | Session ID required | Connection IS the session |
//! | Complex correlation via maps | Simple request-response over single socket |
//!
...
//! ✅ Security validation (Origin header)
...
//! use turbomcp_macros::mcp_server;
//!
//! #[mcp_server]
//! struct MyServer;
//!
...
//!     MyServer.run_websocket("127.0.0.1:8080").await
//! }
//! ```
...
/// use std::sync::Arc;
/// use tokio::sync::mpsc;
/// use turbomcp::runtime::websocket_server::WebSocketServerDispatcher;
///
/// # async fn example() {
/// let (tx, rx) = mpsc::unbounded_channel();
/// let dispatcher = WebSocketServerDispatcher::new(tx);
/// // Use with BidirectionalWrapper
/// # }
/// ```
#[derive(Clone)]
...
impl WebSocketServerDispatcher {
    ...
    /// # Arguments
    ///
    /// * `sender` - Channel to send WebSocket messages
    /// * `pending_requests` - Shared map for correlating responses
    pub fn new(
        sender: mpsc::UnboundedSender<WsMessage>,
        pending_requests: PendingRequests,
    ) -> Self {
...
///
/// This function manages the lifecycle of a single WebSocket connection:
/// 1. Creates dispatcher for server→client requests
/// 2. Creates wrapper with base handler + dispatcher
/// 3. Spawns concurrent tasks for send/receive
/// 4. Handles messages bidirectionally until connection closes
///
/// ## Architecture
///
/// Each WebSocket connection:
/// - Gets its own dispatcher (for server→client requests)
/// - Creates a wrapper combining base handler + dispatcher
/// - Runs until connection closes
///
/// This is simpler than HTTP because WebSocket is per-connection lifecycle
    ...
    W: JsonRpcHandler + Send + Sync + 'static,
{
    ...
    // Create wrapper with base handler + dispatcher
...
/// ## Message Types
///
/// 1. **Client→Server requests**: Process through handler.handle_request()
/// 2. **Responses to server-initiated requests**: Match by ID and complete oneshot channel
...
/// messages with full bidirectional communication support.
///
/// ## Architecture
///
/// Each WebSocket connection:
...
mod tests {
    ...
    async fn test_websocket_dispatcher_creation() {
        ...
        let dispatcher = WebSocketServerDispatcher::new(tx, pending);
